...
export default function AIWebBuilder() {
  ...
  const downloadHTML = useCallback(() => {
    if (!code) return;
    const blob = new Blob([`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"><\/script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet"></head><body>${code}</body></html>`], { type: 'text/html' });
  ...
  const iframeContent = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"><\/script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet"><style>body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }</style></head><body>${code || '<div style="height:100vh; display:flex; align-items:center; justify-content:center; color:#94a3b8; flex-direction: column; gap: 20px;"><h1 style="font-size: 20px;">✨ Create magic</h1><p>Enter a prompt above</p></div>'}</body></html>`;
    ...
    <div className={`flex flex-col h-screen w-screen overflow-hidden ${darkMode ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <aside className={`fixed left-0 top-0 bottom-0 w-80 z-50 transform transition-transform duration-300 flex flex-col border-r ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} ${darkMode ? 'bg-slate-900/95 border-slate-800' : 'bg-white border-slate-200'}`}>
        <div className={`p-4 border-b flex items-center justify-between ${darkMode ? 'border-slate-800' : 'border-slate-200'}`}>
          <div className="flex items-center gap-2 font-bold"><Sparkles size={20} className="text-blue-500" /> EMERGE</div>
